...
app.post('/edit/commit', function (req, res){
  ...
  var edit = "UPDATE itemsEntered Set amount = '" + amount + "' WHERE itemID = (SELECT itemID FROM itemsEntered WHERE name = '" + name + "' AND userID = '" + ID + "');";
  var del = "Delete from itemsEntered where itemID = (SELECT itemID FROM itemsEntered WHERE name = '" + name + "' AND userID = '" + ID + "');";
